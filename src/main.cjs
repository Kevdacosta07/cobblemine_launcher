...
let updates,win,root,settings,selection,installed,auth,game,operation=null,lastProgress=null,logFile;
const uiUrl=pathToFileURL(path.join(__dirname,'../ui/index.html')).href;
const emit=event=>{if(event.type==='progress')lastProgress=event;if(win&&!win.isDestroyed())win.webContents.send('cobblemine:event',event);if(event.type==='game-exit'){if(event.code)log('Minecraft terminé avec le code '+event.code);broadcast();}};
...
function handle(name,fn){ipcMain.handle('cobblemine:'+name,async(event,...args)=>{try{if(event.sender!==win.webContents||event.senderFrame!==win.webContents.mainFrame||event.senderFrame.url.split('#')[0]!==uiUrl)throw Error('Appel non autorisé.');return {ok:true,value:await fn(...args)};}catch(e){return {ok:false,error:cleanError(e)};}});}
...
function wire(){
 handle('check-launcher-update',()=>updates.check());
